...
terraform init -backend-config="resource_group_name=$deploymentResourceGroupName" -backend-config="storage_account_name=$deploymentStorageAccountName" -backend-config="key=posmockapiterraform.deployment.tfplan"
...
terraform plan -out "posmockapiterraform.deployment.tfplan"
if ( !$? ) { echo "Something went wrong during terraform plan" ; throw "Error" }

Write-output "Executing terraform apply"
terraform apply  "posmockapiterraform.deployment.tfplan"
...
Write-Host "##vso[task.setvariable variable=fm_mock_web_app_url]$($terraformOutput.fm_mock_web_app_url.value)"
