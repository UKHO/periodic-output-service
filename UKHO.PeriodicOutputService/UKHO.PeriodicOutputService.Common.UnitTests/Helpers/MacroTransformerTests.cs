...
namespace UKHO.PeriodicOutputService.Common.UnitTests.Helpers
{
    ...
    public class MacroTransformerTests
    {
        ...
        public static object[] macroValues =
        {
           ...
           new string[2]{ "$(now.WeekNumber +1)", WeekNumber.GetUKHOWeekFromDateTime(today.AddDays(1 * 7)).Week.ToString().PadLeft(2,'0')},
           new string[2]{ "$(now.WeekNumber.Year)", WeekNumber.GetUKHOWeekFromDateTime(today.AddDays(1 * 7)).Year.ToString()},
