...
public class AzureMessageQueueHelper : IAzureMessageQueueHelper
{
    ...
    public async Task AddMessageAsync(string message, string configName, string fileName, string messageID)
    {
        ...
        logger.LogInformation(EventIds.BessConfigPropertiesAddedInQueue.ToEventId(), "Added message in Queue:{queue}, Filename:{filename}, Configname:{configname}, MessageID:{messageID} and _X-Correlation-ID:{CorrelationId}", bessStorageConfiguration.Value.QueueName, fileName, configName, messageID, CommonHelper.CorrelationID);
    }
}
