...
public class AzureMessageQueueHelper : IAzureMessageQueueHelper
{
    ...
    public async Task AddMessageAsync(string message, string configName, string fileName, string builderService_CorrelationId)
    {
        ...
        logger.LogInformation(EventIds.BessConfigPropertiesAddedInQueue.ToEventId(), "Added message in Queue:{queue}, Filename:{filename}, Configname:{configname}, BuilderService_X-Correlation-ID:{builderService_CorrelationId} and _X-Correlation-ID:{CorrelationId}", bessStorageConfiguration.Value.QueueName, fileName, configName, builderService_CorrelationId, CommonHelper.CorrelationID);
    }
}
