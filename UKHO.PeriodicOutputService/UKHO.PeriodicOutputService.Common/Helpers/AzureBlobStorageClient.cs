...
namespace UKHO.PeriodicOutputService.Common.Helpers
{
    ...
    public class AzureBlobStorageClient : IAzureBlobStorageClient
    {
        ...
        public Dictionary<string, string> GetConfigsInContainer()
        {
            ...
            {
                logger.LogError(EventIds.BessErrorOccurredWhileDownloadingConfigFromAzureStorage.ToEventId(), "Exception occurred while downloading configs of json file from azure storage container with Exception Message : {Message} | StackTrace : {StackTrace} | _X-Correlation-ID : {CorrelationId}", ex.Message, ex.StackTrace, CommonHelper.CorrelationID);
            }
            return configs;
        }
