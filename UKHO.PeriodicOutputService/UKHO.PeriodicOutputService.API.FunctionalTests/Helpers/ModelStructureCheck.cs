...
namespace UKHO.PeriodicOutputService.API.FunctionalTests.Helpers
{
    public static class ModelStructureCheck
    {
        ...
        public static async Task CheckModelStructureForSuccessResponseAio(this HttpResponseMessage apiResponse)
        {
            ...
            apiResponseData.Links.AioExchangeSetFileUri.Href.Should().NotBeNull();
            Uri.IsWellFormedUriString(apiResponseData.Links.ExchangeSetFileUri.Href, UriKind.RelativeOrAbsolute).Should().BeTrue();
            ...
            apiResponseData.RequestedAioProductCount.GetType().Equals(typeof(int)).Should().BeTrue();
            apiResponseData.RequestedAioProductCount.Should().Be(1);

            //Check data type of AioExchangeSetCellCount and value should not be less than zero
            apiResponseData.AioExchangeSetCellCount.GetType().Equals(typeof(int)).Should().BeTrue();
            apiResponseData.AioExchangeSetCellCount.Should().BeGreaterThanOrEqualTo(0);
